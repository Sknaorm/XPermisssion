...
class OneSelector(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    init {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.OneSelector)
        mBindId = ta.getResourceId(R.styleable.OneSelector_bind_id, -1)
        isBindViewSelected = ta.getBoolean(R.styleable.OneSelector_isSelected, false)
        val selectedTextColor = ta.getColor(R.styleable.OneSelector_selected_text_color, -1)
        if (selectedTextColor != -1) {
            mSelectedTextColor = ColorStateList.valueOf(selectedTextColor)
        }
        val defaultTextColor = ta.getColor(R.styleable.OneSelector_default_text_color, -1)
        if (defaultTextColor != -1) {
            mDefaultTextColor = ColorStateList.valueOf(defaultTextColor)
        }
        mSelectedDrawable = ta.getDrawable(R.styleable.OneSelector_selected_drawable)
        val selectedStyle = ta.getResourceId(R.styleable.OneSelector_selected_style, -1)
        if (selectedStyle != -1) {
            mSelectedDrawable = AppearanceUtils.getDrawableByAttrs(context, selectedStyle)
        }
        //style文件
        mDefaultDrawable = ta.getDrawable(R.styleable.OneSelector_default_drawable)
        val defaultStyle = ta.getResourceId(R.styleable.OneSelector_default_style, -1)
        if (defaultStyle != -1) {
    ...
    override fun onAttachedToWindow() {
        ...
        setBindViewOneSelector()
    }

    private fun setBindViewOneSelector() {
